...
final class FreeTimeStore: ObservableObject {
    ...
    func add(_ plan: TimePlan) { plans.append(plan); save() }
    ...
    func delete(_ plan: TimePlan) {
        if let key = templatePlanKey(for: plan) { deletedTemplatePlanKeys.insert(key) }
    ...
    func delete(_ template: RoutineTemplate) {
        ...
        deletedTemplatePlanKeys = deletedTemplatePlanKeys.filter { !$0.hasPrefix("\(template.id.uuidString)|") }
        plans.removeAll {
    ...
    func scheduledMinutes(for task: FreeTimeTask) -> Int {
        ...
        return merged.reduce(0) { $0 + Int($1.duration / 60) }
    }
    ...
    func freeMinutes(on date: Date) -> Int {
        ...
        let dayStart = calendar.startOfDay(for: date)
        let dayEnd = calendar.date(byAdding: .day, value: 1, to: dayStart) ?? dayStart
        return freeMinutes(from: dayStart, to: dayEnd)
    }

    func freeMinutes(from start: Date, to end: Date) -> Int {
    ...
    func availableSlots(before deadline: Date, minimumMinutes: Int = 30) -> [DateInterval] {
        ...
        return (0...dayCount).flatMap { offset -> [DateInterval] in
            ...
            let dayEnd = min(calendar.date(byAdding: .day, value: 1, to: date) ?? deadline, deadline)
    ...
    private func apply(_ template: RoutineTemplate, replacingExisting: Bool) {
        ...
        if replacingExisting {
            plans.removeAll { $0.sourceTemplateID == template.id && $0.start >= today }
        }
        ...
        for offset in 0..<dayCount {
            ...
            for item in template.items {
                ...
                let deletionKey = templatePlanKey(templateID: template.id, itemID: item.id, date: day)
    ...
    private func pruneAutomaticBackups(in directory: URL) {
        ...
        let oldBackups = urls
            ...
            .sorted { modificationDate(of: $0) > modificationDate(of: $1) }
            .dropFirst(10)
        for url in oldBackups { try? FileManager.default.removeItem(at: url) }
    }
    ...
    private func updateWidgetSnapshot() {
        ...
        let snapshot = WidgetSnapshot(
            ...
            onPlans: plans
                ...
                .sorted { $0.start < $1.start }
                .prefix(100)
                .map { WidgetPlanSnapshot(title: $0.title, start: $0.start, end: $0.end) }
        )
    ...
    private func templatePlanKey(for plan: TimePlan) -> String? {
        guard let templateID = plan.sourceTemplateID, let itemID = plan.sourceTemplateItemID else { return nil }
